...
const UserLogs = ({ userID, logs }: UserProp) => {
  ...
  return (
    <>
      {logs && (
        <article className="laptop:w-[90%] min-h-[300px] h-full w-full mb-auto mt-6 flex flex-col laptop:block laptop:ml-[42px]">
          {missedSessions.length > 0 && (
            <>
              <h4 className="p-1 font-semibold laptop:text-lg laptop:ml-0 ml-[5%]">
                Missed sessions for{' '}
                <span className="capitalize">
                  <TeamFetch teamID={logs[0].team_id} />
                </span>
                :
              </h4>
          ...
          {userFilledSessions.length > 0 && (
            <>
              <h4 className="p-1 font-medium pt-2 laptop:text-lg laptop:ml-0 ml-[5%]">
                Filled in sessions for{' '}
                <span className="capitalize">
                  <TeamFetch teamID={logs[0].team_id} />
                </span>
                :
              </h4>
              <div className="flex flex-col items-center h-full mb-4 overflow-y-auto">
                {userFilledSessions
                  .sort(
                    (a, b) =>
                      new Date(b.session_date).getTime() -
